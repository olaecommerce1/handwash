...
class ActionSteps:
    ...
    def is_correct_step(self, step_just_completed):
        ...
        if "or" in self.get_next_step():
            if step_just_completed == self.get_next_step().split(" or ")[0] or step_just_completed == \
                    self.get_next_step().split(" or ")[1]:
                return True
        else:
    ...
    def get_next_step(self):
        ...
        if ("LEFT" in list(self.steps.keys())[self.current_step_pointer]) and \
            ...
            return str(list(self.steps.keys())[self.current_step_pointer]) + " or " + str(
                list(self.steps.keys())[self.current_step_pointer + 1])
